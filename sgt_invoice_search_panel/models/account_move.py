...
class AccountMove(models.Model):
    ...
    def get_views(self, views, options=None):
        ...
        if 'search' in res['views']:
            ...
            active_fields = self.env['sgt.invoice.search.panel.field'].search([('is_active', '=', True)])
            
            for field in active_fields:
                field_name = field.field_id.name
                
                # Bơm trường vào metadata nếu Odoo chưa tự động load
                if field_name not in res['models'].get('account.move', {}):
                    field_info = self.fields_get([field_name])
                    if field_name in field_info:
                        res['models'].setdefault('account.move', {}).update(field_info)
